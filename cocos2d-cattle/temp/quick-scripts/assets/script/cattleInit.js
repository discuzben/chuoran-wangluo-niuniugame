(function() {"use strict";var __module = CC_EDITOR ? module : {exports:{}};var __filename = 'preview-scripts/assets/script/cattleInit.js';var __require = CC_EDITOR ? function (request) {return cc.require(request, require);} : function (request) {return cc.require(request, __filename);};function __define (exports, require, module) {"use strict";
cc._RF.push(module, '64f853EBD5M8K64qb/Ahu92', 'cattleInit', __filename);
// script/cattleInit.js
...
var odds = null;
// 扑克牌数字相对于扑克牌的坐标
var pokernumposi = [{ x: -42, y: 62 }];
//庄家倍数和玩家下注显示坐标
var userchooseposi = [{ x: 0, y: 0 }, { x: 0, y: 0 }, { x: 0, y: 0 }, { x: 0, y: 0 }];
//扑克牌的坐标
var pokerposi = [{ x: 515, y: -379 }, { x: 689, y: -379 }, { x: 863, y: -379 }, { x: 1037, y: -379 }, { x: 1211, y: -379 }, { x: 0, y: 215 }, { x: 60, y: 215 }, { x: 120, y: 215 }, { x: 180, y: 215 }, { x: 240, y: 215 }, { x: 796, y: 473 }, { x: 854, y: 473 }, { x: 912, y: 473 }, { x: 970, y: 473 }, { x: 1028, y: 473 }, { x: 1490, y: 215 }, { x: 1549, y: 215 }, { x: 1607, y: 215 }, { x: 1665, y: 215 }, { x: 1722, y: 215 }];
var sortpokerposi = [{ x: 664, y: -379 }, { x: 744, y: -379 }, { x: 824, y: -379 }, { x: 982, y: -379 }, { x: 1062, y: -379 }, { x: 0, y: 215 }, { x: 80, y: 215 }, { x: 160, y: 215 }, { x: 292, y: 215 }, { x: 372, y: 215 }, { x: 796, y: 473 }, { x: 876, y: 473 }, { x: 956, y: 473 }, { x: 1082, y: 473 }, { x: 1162, y: 473 }, { x: 1350, y: 215 }, { x: 1430, y: 215 }, { x: 1510, y: 215 }, { x: 1642, y: 215 }, { x: 1722, y: 215 }];
var mnsortpoker = [{ x: 703, y: -379 }, { x: 783, y: -379 }, { x: 863, y: -379 }, { x: 943, y: -379 }, { x: 1023, y: -379 }, { x: 0, y: 215 }, { x: 80, y: 215 }, { x: 160, y: 215 }, { x: 240, y: 215 }, { x: 320, y: 215 }, { x: 796, y: 473 }, { x: 876, y: 473 }, { x: 956, y: 473 }, { x: 1036, y: 473 }, { x: 1116, y: 473 }, { x: 1402, y: 215 }, { x: 1482, y: 215 }, { x: 1562, y: 215 }, { x: 1642, y: 215 }, { x: 1722, y: 215 }];
// 用于保存金币生成的位置
var coinposi = [{ x: -783, y: -361 }, { x: -771, y: 184 }, { x: -175, y: 373 }, { x: 773, y: 181 }];
//用于保存闹钟生成的位置
var clockposi = [{ x: 0, y: 0 }, { x: -426, y: 56 }, { x: -230, y: 268 }, { x: 411, y: 56 }];
//用于保存玩家信息
var playerIds = [];
//用于保存除庄家以外玩家的信息
var exceptbanker = [];
...
var usertoken = [];
// 当前已创建扑克牌的数量
var pokerfacecount = 0;

var updatedata = null;
var updatePokerFacecount = 0;
//玩家选牌的数量
var pokerfaceclickcount = 0;
// 玩家选牌的类型
var _clickpokertype = [];
// 用于控制倒计时
var allcount = 0;
...
var bankerisclick = false;
// 当前玩家的金币数
var curplexperience = 0;
...
cc.Class({
    ...
    playAudio: function playAudio(name, isMan) {
        if (isMan == null) {} else {
            if (isMan === true) {
                name += '_man';
            } else {
    ...
    defaultscene: function defaultscene() {
        // 设置抢庄按钮默认隐藏
    ...
    //创建倒计时数字资源
    spawncountdown: function spawncountdown(count) {
    ...
    //创建庄家倍数资源
    spawnbankerOdds: function spawnbankerOdds(bankerindex, odds) {
    ...
    //创建除庄家外其余玩家下注倍数资源
    spawnplayerOdds: function spawnplayerOdds(indexof, odds) {
    ...
    // 创建扑克牌正面资源
    spawnpokerface: function spawnpokerface(color, num, pX, pY, pfc, roomKey) {
    ...
    clickpokertype: function clickpokertype(type) {
        if (type == 'J') {
            _clickpokertype.push(10);
        } else if (type == 'Q') {
            _clickpokertype.push(10);
        } else if (type == 'K') {
            _clickpokertype.push(10);
        } else {
            _clickpokertype.push(type);
        }
    },

    //创建牛几资源
    spawnbulltype: function spawnbulltype(posiindex, typeindex) {
    ...
    //创建输赢数字
    spawnwinfailnum: function spawnwinfailnum(firstsymbol, symbol, plindex, j) {
    ...
    spawncoin: function spawncoin(x, y) {
    ...
    //根据已选牌推送显示牌面
    updatePokerFace: function updatePokerFace(pId, data) {
        var _this = this;
        ...
        if (typeof usertoken[drawindex] !== 'string') {
            ...
            data.pokes[ut].pokes.forEach(function (item, index) {
                if (typeof item === 'string') {

                    var pf = eval('(' + item + ')');
                    _this.spawnpokerface(pf.color, pf.num, pokerposi[updatePokerFacecount].x, pokerposi[updatePokerFacecount].y, 7);
                } else {
                    _this.spawnpokerface(item.color, item.num, pokerposi[updatePokerFacecount].x, pokerposi[updatePokerFacecount].y, 7);
                }
                updatePokerFacecount++;
            });
        } else {
            data.pokes[usertoken[drawindex]].pokes.forEach(function (item, index) {
                if (typeof item === 'string') {
    ...
    updateLocalCoins: function updateLocalCoins(localSn, ownCoins) {
        // if(localSn == 3){
        GD.player.experience = ownCoins;
        // }
    },
    gameOverDeal: function gameOverDeal(data) {
        var self = this;
        self.jxyx.active = true;

        //加钱扣钱操作
        data.players.forEach(function (item) {
            var localSn = self._seatNumMatch(item.seatNum);
            var userNode = self.childNodes['user' + localSn];
            var moneyNode = userNode.getChildByName('moneyNode');
    ...
    //定义EventHandler
    myHandler: function myHandler(name) {
    ...
    //对应myHandler的处理方法
    handlerFunc: function handlerFunc(event, name) {
        if (name == 'back_hall') {
            this.back();
    ...
    bankertime: function bankertime(timeout) {
        allcount = timeout;
        this.schedule(this.updateTime, 1, allcount, 0);
    },

    //匹配房间
    matchingRoom: function matchingRoom() {
        // console.log('匹配中...');
        var cb = function cb() {
            //提示后执行的函数，可为null
            cc.log('modal - tip');
    ...
    onGameEvent: function onGameEvent() {
        ...
        ZR_IO.on('game_unfinish', function (data, status) {
            ccUtil.modalLoading(data.msg, {
                buttons: [{ name: 'back', handler: self.myHandler('back_hall') }]
            });
        });
        ...
        //通知谁可以进入房间->通知服务端进入
        ZR_IO.on('game_ready', function (data, status) {
            //房间匹配成功的事件
            if (status) {
        ...
        ZR_IO.on('choose_yn', function (data, status) {
            // console.log(data);
            if (playerIds.indexOf(data.playerId) == 0) {
                //自己选牌自己的牌就不做操作
            } else {
                // console.log(data.playerId);
                var drawindex = playerIds.indexOf(data.playerId);
                _this2.wc.children[drawindex].active = true;
                // this.scheduleOnce(function() {
                //     this.updatePokerFace(data.playerId, updatedata);
                // }, getRandom(0, 1));
            }
        });
        ZR_IO.on('choose_mn', function (data, status) {
            // console.log(data);
            if (playerIds.indexOf(data.playerId) == 0) {
                //自己选牌自己的牌就不做操作
            } else {
        ...
        ZR_IO.on('game_begin', function (data, status) {
            //游戏开始前获取用户的信息
            if (status) {
                ccUtil.closeModal();
                ...
                _this2.roomKey.getComponent("cc.Label").string = data.roomInfo.roomId;

                var _loop = function _loop() {
                    // 设置玩家在房间中显示的信息
                    _this2.userinfo[i].getChildByName('uid').getComponent("cc.Label").string = data.playersInfo[playerIds[i]].username;
                    account = data.playersInfo[playerIds[i]].experience.toFixed(1);
                    a = parseFloat(account);

                    _this2.userinfo[i].getChildByName('account').getComponent("cc.Label").string = a;

                    //设置玩家头像
                    var headimgurl = data.playersInfo[playerIds[i]].headimg;
                    var headNode = _this2.mask[i].getChildByName('userhead');

                    if (headimgurl) {
                        cc.loader.load(headimgurl, function (err, item) {
                ...
                _this2.yxks.active = true;
                cc.audioEngine.play(_this2.audiosource.getComponent('audio').gamestart, false, 1);

                // 设置开始游戏节点动画
                var scaleaction = cc.sequence(cc.show(), cc.scaleTo(0.3, 0.5, 0.5), cc.delayTime(3), cc.hide());
                _this2.yxks.runAction(scaleaction);
            }
        });

        ZR_IO.on('banker_roll', function (data, status) {
            //游戏开始，抢庄
            // console.log(data);
            if (status) {
                step = 1;
                ...
                if (curplexperience < 480) {
                    for (var i = 1; i < _this2.snatchvillagebtn.children.length - 1; i++) {
                        _this2.snatchvillagebtn.children[i].getComponent(cc.Button).interactable = false;
                        _this2.snatchvillagebtn.children[i].opacity = 127.5;
                        _this2.snatchvillagebtn.children[i].targetOff(_this2);
                    }
                } else if (curplexperience < 720) {
                    for (var i = 2; i < _this2.snatchvillagebtn.children.length - 2; i++) {
                        _this2.snatchvillagebtn.children[i].getComponent(cc.Button).interactable = false;
                        _this2.snatchvillagebtn.children[i].opacity = 127.5;
                        _this2.snatchvillagebtn.children[i].targetOff(_this2);
                    }
                } else if (curplexperience < 960) {
                    for (var i = 3; i < _this2.snatchvillagebtn.children.length - 3; i++) {
                        _this2.snatchvillagebtn.children[i].getComponent(cc.Button).interactable = false;
                        _this2.snatchvillagebtn.children[i].opacity = 127.5;
                        _this2.snatchvillagebtn.children[i].targetOff(_this2);
                    }
                ...
                if (playerIds.length == 2) {
                    _self.scheduleOnce(function () {
                        ...
                        _self.schedule(_self.updateTime, 1, allcount, 0);
                    }, 0);
                    _self.scheduleOnce(function () {
                        this.unschedule(_self.updateTime);
                        console.log("2");
                        clock = 1;
                        allcount = data.timeout;
                        _self.schedule(_self.updateTime, 1, allcount, 0);
        ...
        ZR_IO.on('one_choosebanker', function (data, status) {
            //下一步：倒计时
            if (status) {
                console.log("one_choosebanker:" + data.playerId);
                var bankerindex = playerIds.indexOf(data.playerId);
                _this2.spawnbankerOdds(bankerindex, data.odds);
            }
        });
        ZR_IO.on('player_roll', function (data, status) {
            //闲家定倍
            if (status) {
                step = 2;
                ispc_deal = true;
                _this2.snatchvillagebtn.active = false;
                // 取消抢庄环节的定时器
                _this2.unschedule(_this2.updateTime);
                // 获取庄家在数据中的排位号,
                ...
                //设置庄家等待抢注提示
                if (playerIds.indexOf(data.banker.playerId) == 0) {
                    //如果庄家是玩家自己，显示等待下注提示
                    _this2.waitchipin.active = true;
                } else {
                    //庄家不是玩家自己，不显示等待下注提示
                    _this2.chipinbtn.active = true;
                    // 根据玩家金币多少显示下注按钮
                    if (curplexperience < 64) {
                        _this2.chipinbtn.getChildByName('chipinbtn1').active = true;
                        _this2.chipinbtn.getChildByName('chipinbtn2').active = false;
                        _this2.chipinbtn.getChildByName('chipinbtn3').active = false;
                        _this2.chipinbtn.getChildByName('chipinbtn4').active = false;
                        _this2.chipinbtn.getChildByName('chipinbtn5').active = false;
                    } else if (curplexperience < 128) {
                        _this2.chipinbtn.getChildByName('chipinbtn1').active = false;
                        _this2.chipinbtn.getChildByName('chipinbtn2').active = true;
                        _this2.chipinbtn.getChildByName('chipinbtn3').active = false;
                        _this2.chipinbtn.getChildByName('chipinbtn4').active = false;
                        _this2.chipinbtn.getChildByName('chipinbtn5').active = false;
                        ...
                        _this2.chipinbtn.getChildByName('chipinbtn1').active = false;
                        _this2.chipinbtn.getChildByName('chipinbtn2').active = false;
                        _this2.chipinbtn.getChildByName('chipinbtn3').active = true;
                        _this2.chipinbtn.getChildByName('chipinbtn4').active = false;
                        _this2.chipinbtn.getChildByName('chipinbtn5').active = false;
                        ...
                        _this2.chipinbtn.getChildByName('chipinbtn2').active = false;
                        _this2.chipinbtn.getChildByName('chipinbtn3').active = false;
                        _this2.chipinbtn.getChildByName('chipinbtn4').active = true;
                        _this2.chipinbtn.getChildByName('chipinbtn5').active = false;
                    } else {
                        _this2.chipinbtn.getChildByName('chipinbtn1').active = false;
                        _this2.chipinbtn.getChildByName('chipinbtn2').active = false;
                        _this2.chipinbtn.getChildByName('chipinbtn3').active = false;
                        _this2.chipinbtn.getChildByName('chipinbtn4').active = false;
                        _this2.chipinbtn.getChildByName('chipinbtn5').active = true;
                    }
                }
                _this2.spawnbankerOdds(bankerindex, data.banker.odds);
                //设置除庄家其余玩家的抢庄倍数
                for (var i = 0; i < playerIds.length; i++) {
                ...
                if (data.bankerChoose) {
                    for (var i = 0; i < exceptbanker.length; i++) {
                        ...
                        _this2.spawnbankerOdds(plindex, data.bankerChoose[exceptbanker[i]].odds);
                    }
                } else {
                    for (var i = 0; i < exceptbanker.length; i++) {
                        //获取playerIds数组中当前循环到的玩家的下标
                        var plindex = playerIds.indexOf(exceptbanker[i]);
                        _this2.spawnbankerOdds(plindex, data.plasyer[exceptbanker[i]].odds);
                    }
                }

                _this2.ddk.active = false;

                // 设置下注倒计时时长
                allcount = data.timeout;
                //设置下注环节倒计时
                clock = 0;
                _this2.schedule(_this2.updateTime, 1, allcount, 0);
            }
        });
        ZR_IO.on('one_chooseplayer', function (data, status) {
            //下一步：倒计时
            if (status) {
                console.log("one_chooseplayer:" + data);
                var plindex = playerIds.indexOf(data.playerId);
                _this2.spawnplayerOdds(plindex, data.odds);
            }
        });
        ZR_IO.on('deal', function (data, status) {
            ...
            if (status) {
                ...
                _this2.chipinbtn.active = false;
                _this2.waitchipin.active = false;
                _this2.dxk.active = true;
                _this2.pokerchoose.active = true;
                _this2.jsk.active = true;
                _this2.dealpoker();
                ...
                for (var i = 0; i < exceptbanker.length; i++) {
                    //获取playerIds数组中当前循环到的玩家的下标
                    var plindex = playerIds.indexOf(exceptbanker[i]);
                    _this2.spawnplayerOdds(plindex, data.plasyer[exceptbanker[i]].odds);
                }
                _this2.scheduleOnce(function () {
                    var _this3 = this;
                    ...
                    if (typeof usertoken[0] !== 'string') {
                        ...
                        data.pokes[ut].pokes.forEach(function (item, index) {
                            if (typeof item === 'string') {
                                ...
                                var pf = eval('(' + item + ')');
                                _this3.spawnpokerface(pf.color, pf.num, pokerposi[pokerfacecount].x, pokerposi[pokerfacecount].y, pokerfacecount, data.roomKey);
                                ...
                                _this3.spawnpokerface(item.color, item.num, pokerposi[pokerfacecount].x, pokerposi[pokerfacecount].y, pokerfacecount, data.roomKey);
                        ...
                        data.pokes[usertoken[0]].pokes.forEach(function (item, index) {
                            if (typeof item === 'string') {
                    ...
                    function choosepoker() {
                        ...
                        var pokermove = [false, false, false, false, false];
                        // 保存牌的下标
                        var cplabel = [];
                        // 保存当前点击数的下标
                        var cplabel1 = [];
                        for (var i = 0; i < 5; i++) {
                            ...
                            self.pokers.children[playerIds.length * 5 + i].on(cc.Node.EventType.TOUCH_END, function (event) {
                                ...
                                var currentpoindex = event.currentTarget.index;
                                ...
                                if (pokermove[currentpoindex]) {
                                    ...
                                    pokerfaceclickcount--;
                                    var ind = cplabel.indexOf(currentnum);
                                    var ind1 = cplabel1.indexOf(currentpoindex);
                                ...
                                if (pokerfaceclickcount >= 3) {
                                    ...
                                    self.jsk.getChildByName('AN-yn').on(cc.Node.EventType.TOUCH_END, function () {
                                        if (_clickpokertype[0] != null && _clickpokertype[1] != null && _clickpokertype[2] != null) {
                                            self.jsk.children[3].getComponent('cc.Label').string = _clickpokertype[0] + _clickpokertype[1] + _clickpokertype[2];
                                            if ((_clickpokertype[0] + _clickpokertype[1] + _clickpokertype[2]) % 10 == 0) {
                                    ...
                                    // 去除已选牌之外其他牌的点击事件
                                    var noclickarr = [0, 1, 2, 3, 4];
                                    console.log(cplabel1);
                                    for (var i = 0; i < cplabel1.length; i++) {
                                        var curind = noclickarr.indexOf(cplabel1[i]);
                            ...
                            if (typeof usertoken[leftInd] !== 'string') {
                                ...
                                data.pokes[ut].pokes.forEach(function (item, index) {
                                    if (typeof item === 'string') {
                                        ...
                                        var pf = eval('(' + item + ')');
                                        _this4.spawnpokerface(pf.color, pf.num, pokerposi[pokerfacecount].x, pokerposi[pokerfacecount].y, pokerfacecount);
                                    } else {
                                        _this4.spawnpokerface(item.color, item.num, pokerposi[pokerfacecount].x, pokerposi[pokerfacecount].y, pokerfacecount);
                                    }
                                    pokerfacecount++;
                                    if ((pokerfacecount + 1) % 5 == 0) {
                                ...
                                data.pokes[usertoken[leftInd]].pokes.forEach(function (item, index) {
                                    if (typeof item === 'string') {
                    ...
                    this.scheduleOnce(function () {
                        ...
                        var _loop2 = function _loop2() {
                            var poke = data.pokes[usertoken[s]].pokes;
                            // 使用冒泡排序法根据牌的大小对牌进行排序
                            for (i = 0; i < poke.length; i++) {
                                for (j = 0; j < poke.length - 1 - i; j++) {
                            ...
                            var ismn = false;
                            for (t = 0; t < usertoken.length; t++) {
                                var curarr = threepoke(t);
                                if (curarr != undefined) {
                                    for (y = 0; y < curarr.length; y++) {
                                        var _temp = poke[y];
                            ...
                            pokerfacecount = s * 5;
                            poke.forEach(function (item, index) {
                                if (typeof item === 'string') {

                                    var pf = eval('(' + item + ')');
                                    if (ismn) {
                                        _this5.spawnpokerface(pf.color, pf.num, mnsortpoker[pokerfacecount].x, mnsortpoker[pokerfacecount].y, pokerfacecount);
                                    } else {
                                        _this5.spawnpokerface(pf.color, pf.num, sortpokerposi[pokerfacecount].x, sortpokerposi[pokerfacecount].y, pokerfacecount);
                                    }
                                } else {
                                    if (ismn) {
                                        _this5.spawnpokerface(item.color, item.num, mnsortpoker[pokerfacecount].x, mnsortpoker[pokerfacecount].y, pokerfacecount);
                                    } else {
                                        _this5.spawnpokerface(item.color, item.num, sortpokerposi[pokerfacecount].x, sortpokerposi[pokerfacecount].y, pokerfacecount);
                                    }
                                }
                                pokerfacecount++;
                                // if ((pokerfacecount + 1) % 5 == 0) {
                    ...
                    this.scheduleOnce(function () {
                        ...
                        for (var i = 0; i < exceptbanker.length; i++) {
                            ...
                            for (var j = 0; j < winfailnum.length; j++) {
                                ...
                                if (plindex == 3) {
                                    var childindex = 6 - winfailnum.length;
                    ...
                    this.scheduleOnce(function () {
                        ...
                        for (var s = 0; s < bankerwinfailnum.length; s++) {
                            ...
                            if (baindex == 3) {
                                var childindex = 6 - bankerwinfailnum.length;
                                console.log(childindex);
                    ...
                    this.scheduleOnce(function () {
                        ...
                        if (data.pokes[data.banker.token].pokeId == comparearr[0]) {
                            // 设置通杀
                            this.ts.active = true;
                            this.ts.runAction(tscaleaction);
                            cc.audioEngine.play(this.audiosource.getComponent('audio').pentakill, false, 1);
                        } else if (data.pokes[data.banker.token].pokeId == comparearr[playerIds.length]) {
                            // 设置通赔
                            this.tp.active = true;
        ...
        ZR_IO.on('game_over', function (data, status) {
            //下一步：倒计时
            if (status) {
                console.log(data);
                var uia = data.userinfo;
                for (var i = 0; i < uia.length; i++) {
                    var cind = playerIds.indexOf(uia[i].playerId);
                    _this2.userinfo[cind].getChildByName('account').getComponent(cc.Label).string = uia[i].experience.toFixed(2);
                }
            }
        });
    },
    ...
    // 提交抢庄倍数
    commitBankerOdds: function commitBankerOdds() {
        var self = this;
    ...
    // 提交下注倍数
    chipinOdds: function chipinOdds() {
        var self = this;
    ...
    onLoad: function onLoad() {
        ...
        cc.director.on('emit-setting-exit', function () {
            self.back();
        });
        //存放数据
        this.data = {};
...
cc._RF.pop();
        ...
        //# sourceMappingURL=cattleInit.js.map
